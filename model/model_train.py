...
for metal_name in ['gold', 'silver', 'palladium', 'platinum']:
    train_dataset = load_dataset('json', data_files=f'../json_train_{metal_name}.jsonl', split='train') # путь к трейну
    eval_dataset = load_dataset('json', data_files=f'../json_valid_{metal_name}.jsonl', split='train') # путь к валиду
    ...
    args = TrainingArguments(
        output_dir=f"model_generation_{metal_name}",
        max_steps=200,
    ...
    model.save_pretrained(f"done_lora_{metal_name}")